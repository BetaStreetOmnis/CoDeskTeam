...
class OpencodeService:
    ...
    async def _ensure_session(self, client: httpx.AsyncClient, st: SessionState) -> str:
        ...
        try:
            res = await client.post("/session", json={"title": "aistaff"})
                ...
                try:
                    ...
                    if perm == "edit":
                        ...
                        if any(isinstance(p, str) and (_is_sensitive_workspace_path(workspace, p) or _is_sensitive_env_path(p)) for p in patterns):
                            reply = "reject"
                            msg = "aistaff: reading sensitive files disabled"
                        else:
                            for p in patterns:
                                ...
                                if not resolved:
                                    reply = "reject"
                                    msg = "aistaff: read outside workspace disabled"
                                    break
    ...
    async def chat(
        ...
        *,
        ...
        # Enforce aistaff toggles via opencode "tools" overrides.
