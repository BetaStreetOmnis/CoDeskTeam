...
from .datasource_store import DatasourceRecord
from .llm import ChatbiLLM, ChatbiLlmConfig
from .remote_db import RemoteDBError, create_engine_from_url, iter_table_rows, load_table
...
def _chart_suggestion(question: str, data: list[dict]) -> dict | None:
    ...
    if any(k in q for k in ["散点", "相关", "关联"]):
    ...
    if any(k in q for k in ["堆叠", "叠加"]):
        cfg = bar_multi("bar_stack")
        if cfg:
            return cfg

    if any(k in q for k in ["对比", "分组", "对照"]):
        cfg = bar_multi("bar_group")
        if cfg:
            return cfg

    if any(k in q for k in ["条形", "横向", "横版"]):
    ...
    if any(k in q for k in ["趋势", "走势", "变化", "按月", "按天"]):
        x = text_cols[0] if text_cols else cols[0]
        if len(numeric_cols) >= 2:
...
def _ensure_limit(sql: str, max_rows: int) -> str:
    if not sql:
        return sql
    if _parse_limit(sql) is not None:
        return sql
    # Naive append; good enough for typical single SELECT queries.
...
class ChatbiSettings:
    demo_db_path: str
    openai_base_url: str
    openai_api_key: str | None
    model: str
    llm_timeout_ms: int = 45_000
    llm_explain_timeout_ms: int = 12_000
    sql_max_rows: int = 500
    sql_timeout_ms: int = 8000
    stream_chunk_chars: int = 48
    stream_chunk_delay_ms: int = 0
    federated_max_rows_per_table: int = 5000
    federated_batch_size: int = 400
...
def _iter_chunks(text: str, size: int) -> list[str]:
    size = max(1, int(size))
    if not text:
        return []
...
class QueryEngine:
    def __init__(self, settings: ChatbiSettings, datasources: list[DatasourceRecord]) -> None:
        ...
        self._datasources = [
            ...
            if src.enabled and src.id and src.tables
        ]
    ...
    async def explain_sql(self, question: str, sql: str) -> str:
        ...
        try:
            reply = await asyncio.wait_for(
                self._llm.explain_sql(question=question, sql=sql),
                timeout=max(0.1, int(self._settings.llm_explain_timeout_ms)) / 1000.0,
            )
        except Exception:
            return ""
    ...
    async def ask(self, question: str, datasource_ids: Sequence[str], history: list[dict]) -> dict:
        if _is_smalltalk(question):
            return {"success": True, "mode": "chat", "reply": _smalltalk_reply(question), "question": question}
        ...
        try:
            raw = await asyncio.wait_for(
                self._llm.generate_sql(prompt=prompt),
                timeout=max(0.1, int(self._settings.llm_timeout_ms)) / 1000.0,
            )
        except Exception:
            raw = ""
        sql = _extract_sql(raw)
        if not sql:
            return {"success": False, "error": "模型未返回SQL（可尝试换个问法或缩小范围）"}
        sql = _ensure_limit(sql, int(self._settings.sql_max_rows))

        if not _is_safe_select(sql):
            return {"success": False, "error": "仅允许执行安全的SELECT查询（模型输出不符合约束）"}
    ...
    async def ask_stream(
        ...
        if _is_smalltalk(question):
            yield {"type": "chat", "reply": _smalltalk_reply(question), "question": question}
            return
        ...
        yield {"type": "analysis_delta", "delta": "正在生成 SQL…\n"}
        ...
        sql = _extract_sql(raw)
        if not sql:
            yield {"type": "error", "error": "模型未返回SQL（可尝试换个问法或缩小范围）"}
            return
        sql = _ensure_limit(sql, int(self._settings.sql_max_rows))

        if not _is_safe_select(sql):
            yield {"type": "error", "error": "仅允许执行安全的SELECT查询（模型输出不符合约束）"}
            return

        yield {"type": "analysis_delta", "delta": "SQL 已生成，正在生成解释并执行查询…\n"}
        ...
        for chunk in _iter_chunks(sql_explain, self._settings.stream_chunk_chars):
            yield {"type": "sql_explain_delta", "delta": chunk}
            if self._settings.stream_chunk_delay_ms:
                await asyncio.sleep(max(0, int(self._settings.stream_chunk_delay_ms)) / 1000.0)
        yield {"type": "sql_explain", "sql_explain": sql_explain}

        if sql:
            for chunk in _iter_chunks(sql, self._settings.stream_chunk_chars):
                yield {"type": "sql_delta", "delta": chunk}
                if self._settings.stream_chunk_delay_ms:
                    await asyncio.sleep(max(0, int(self._settings.stream_chunk_delay_ms)) / 1000.0)
            yield {"type": "sql", "sql": sql}

        yield {"type": "analysis_delta", "delta": "SQL 已提交执行，正在汇总结果…\n"}
        ...
        if result.get("success"):
            ...
            if analysis_text:
                for chunk in _iter_chunks(analysis_text, self._settings.stream_chunk_chars):
                    yield {"type": "analysis_delta", "delta": chunk}
                    if self._settings.stream_chunk_delay_ms:
                        await asyncio.sleep(max(0, int(self._settings.stream_chunk_delay_ms)) / 1000.0)
    ...
    def _load_remote_table(
        ...
        try:
            ...
            for batch in iter_table_rows(engine, remote_table.table, batch_size, max_rows=max_rows):
                values = [tuple(self._normalize_remote_value(v) for v in row) for row in batch]
