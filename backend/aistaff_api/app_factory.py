...
def _mount_chatbi(app: FastAPI, settings: Settings) -> None:
    ...
    It is intentionally isolated from aistaff's JWT/team auth for now.
    """
    raw = (os.getenv("AISTAFF_ENABLE_CHATBI") or "").strip().lower()
    # Disabled by default (aistaff now provides a first-class, JWT/team authenticated ChatBI under `/api/chatbi/*`).
    enabled = raw in {"1", "true", "yes", "y", "on"}
    if not enabled:
        return

    vendor_root = (settings.app_root / "backend" / "aistaff_api" / "vendor" / "smart_ask_data").resolve()
...
def _mount_ui(app: FastAPI, settings: Settings) -> None:
    ...
    def root() -> HTMLResponse:
        web_port = (os.getenv("AISTAFF_WEB_PORT") or "5173").strip() or "5173"
...
def _mount_ui_static(app: FastAPI, settings: Settings) -> bool:
    ui_dist_dir = (os.getenv("AISTAFF_UI_DIST_DIR") or "").strip()
