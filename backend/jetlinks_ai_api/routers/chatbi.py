...
def _chatbi_settings(settings: Settings) -> ChatbiSettings:
    ...
    model = (env_str("CHATBI_MODEL", "") or "").strip() or settings.model
    llm_timeout_ms = env_int("CHATBI_LLM_TIMEOUT_MS", 45_000)
    llm_explain_timeout_ms = env_int("CHATBI_EXPLAIN_TIMEOUT_MS", 12_000)
    sql_max_rows = env_int("CHATBI_SQL_MAX_ROWS", 500)
    sql_timeout_ms = env_int("CHATBI_SQL_TIMEOUT_MS", 8_000)
    stream_chunk_chars = env_int("CHATBI_STREAM_CHUNK_CHARS", 48)
    stream_chunk_delay_ms = env_int("CHATBI_STREAM_CHUNK_DELAY_MS", 0)
    federated_max_rows_per_table = env_int("CHATBI_FEDERATED_MAX_ROWS_PER_TABLE", 5_000)
    federated_batch_size = env_int("CHATBI_FEDERATED_BATCH_SIZE", 400)
