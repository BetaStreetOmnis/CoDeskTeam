...
class AgentService:
    ...
    def _codex_skill_hint(self) -> str:
        script_path = (self._settings.app_root / "scripts" / "jetlinks_ai_skill_runner.py").resolve()
        venv_python = (self._settings.app_root / "backend" / ".venv" / "bin" / "python").resolve()
        python_cmd = str(venv_python) if venv_python.exists() else "python3"
        return (
            "【技能脚本】如需生成 PPT/报价/报检/原型文件，请在当前工作区执行：\n"
            f"- 脚本：{script_path}\n"
            f"- 建议命令：{python_cmd} {script_path} <mode> --payload-file /tmp/jetlinks_ai_payload.json\n"
    ...
    async def chat(
        ...
        *,
        ...
        if provider_name == "codex":
            # Keep JetLinks AI session ownership/TTL, but delegate execution to Codex CLI.
            ...
            # Codex CLI doesn't support JetLinks AI's doc/prototype tools.
            # For these requests, fall back to JetLinks AI's own agent loop (OpenAI provider + tools)
        ...
        if provider_name == "opencode":
            # Keep JetLinks AI session ownership/TTL, but delegate the agent loop to OpenCode.
            session = await store.get_or_create(
                session_id=sid,
                user_id=user_id,
                team_id=team_id,
            ...
            # For OpenCode, pass team skills as extra system instruction (avoid mentioning JetLinks AI-specific tools).
            extra_system = ""
            if team_skills_prompt and team_skills_prompt.strip():
                extra_system = team_skills_prompt.strip()
            if summary_text:
            ...
            # OpenCode provider doesn't know JetLinks AI's document/prototype tools. For doc/proto requests,
            # fall back to JetLinks AI's own agent loop so users can trigger file generation via natural language.
            is_ppt, ppt_pages = _ppt_request(message)
            is_quote = _quote_request(message)
            inspect_fmt = _inspection_request(message)
            is_inspection = inspect_fmt is not None
        ...
        if provider_name == "pi":
            if not bool(getattr(self._settings, "enable_pi", False)):
                raise ValueError("Pi provider is disabled. Set JETLINKS_AI_ENABLE_PI=1 to enable it.")
            ...
            # Pi doesn't know JetLinks AI's artifact tools; fall back to JetLinks AI agent loop for doc/proto/attachments.
            if is_ppt or is_quote or is_inspection or is_proto or has_attachments:
                requested = []
                if is_ppt:
                    requested.append("ppt")
        ...
        if provider_name == "nanobot":
            # Keep JetLinks AI session ownership/TTL, but delegate the agent loop to Nanobot.
            session = await store.get_or_create(
                session_id=sid,
                user_id=user_id,
                team_id=team_id,
            ...
            # For Nanobot, pass team skills as extra system instruction (avoid mentioning JetLinks AI-specific tools).
            extra_system = ""
            if team_skills_prompt and team_skills_prompt.strip():
                extra_system = team_skills_prompt.strip()
            if summary_text:
            ...
            # Nanobot provider doesn't know JetLinks AI's document/prototype/attachment tools.
            # For these scenarios, fall back to JetLinks AI's own agent loop.
            is_ppt, ppt_pages = _ppt_request(message)
            is_quote = _quote_request(message)
            inspect_fmt = _inspection_request(message)
            is_inspection = inspect_fmt is not None
            ...
            # If no model is specified per-request, Nanobot will use JETLINKS_AI_MODEL.
            result = await svc.chat(
                session=session,
                message=message,
                role=role,
