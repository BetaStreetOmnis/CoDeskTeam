...
def resolve_project_path(settings: Settings, input_path: str) -> Path:
    ...
    if not path_under_any_root(resolved, resolved_roots):
        raise ValueError(f"项目路径不在允许范围（JETLINKS_AI_PROJECTS_ROOT）：{resolved}")
    if not resolved.exists():
        raise ValueError(f"项目路径不存在：{resolved}")
    if not resolved.is_dir():
        raise ValueError(f"项目路径不是目录：{resolved}")
    reserved = {".aistaff", ".jetlinks-ai"}
    if reserved.intersection({part.lower() for part in resolved.parts}):
        raise ValueError(f"项目路径不能位于内部目录（{', '.join(sorted(reserved))}）")
    return resolved
