...
- **交付型团队**：把需求快速变成 PPT、报价单、报检单、原型和可复用交付物
- **产品与研发团队**：把聊天、仓库上下文、Prompt 技能和受控工具执行放进一个工作台
- **AI 平台 / 运维团队**：集中管理 OpenClaw 相关资源、外部通道和运行能力边界
- **内部效率团队**：沉淀团队 Prompt、项目上下文、生成文件和执行轨迹

## 典型使用场景

| 场景 | JetLinks AI 可以帮你做什么 |
| --- | --- |
| 从需求到交付物 | 从对话开始，直接生成 PPT、报价单、报检单、海报或原型 |
| 受控代码工作台 | 切到 `codex` 或 `opencode`，做仓库内工程任务和受控工具执行 |
| 团队 Prompt 运营 | 把团队技能、项目上下文、需求流转统一沉淀到一个空间里 |
| OpenClaw 运营驾驶舱 | 查看网关状态、同步频道/插件/技能、管理团队级 OpenClaw 资源 |
| 面向业务的 ChatBI | 用自然语言问数，拿到 SQL、表格和分析结果 |
| 带浏览器的研究协作 | 打开网页、跳转、截图，并把结果沉淀到当前会话里 |

## 快速导航

- **预览**：先看产品图和主要界面
- **平台功能总览**：按业务模块快速理解平台能力
- **提供商、档案与安全档位**：理解 provider、会话档案和权限边界
