...
CoDeskTeam は、チーム向けの **セルフホスト可能な AI ワークスペース** です。  
**対話（チャット）駆動の実行** と **チーム運用（権限/分離）** を中心に設計しています。

> 注: 現在のコードベース内部では `aistaff` 命名（環境変数 `AISTAFF_*`、Python パッケージ `aistaff_api`）を引き続き使用しています。利用には影響しません。
...
- サーバ側の許可範囲：`AISTAFF_PROJECTS_ROOT` が、チームが追加できるディレクトリ白リストを定義します（既定は `AISTAFF_WORKSPACE`）
...
- チャット時の適用：
  ...
  - `project_id` なし：チームのワークスペース（`/api/team/settings`）を使うか、未設定なら `AISTAFF_WORKSPACE` にフォールバック

推奨:

- 同じリポジトリを「共有」したい場合は、各チームの `team_projects` に同一パスを追加します（設定がチームごとに分離されるだけで、ファイルをコピーしません）
- 機密は置かず、運用では `AISTAFF_ENABLE_WRITE=0` を基本にして参照用（読み取り中心）にするのがおすすめです
...
- `download_url` は通常 **相対パス**（例: `/api/files/...`）。`AISTAFF_PUBLIC_BASE_URL` を設定すると絶対 URL になります。
